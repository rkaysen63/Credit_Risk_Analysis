...
## Overview:
The purpose of this analysis is evaluate six supervised machine learning models for their ability to predict high risk loans.  The models will be trained using an unbalanced dataset from LendingClub lending services. (The ratio of low risk loans to high risk loans in the dataset used for this analysis is nearly 200:1.)
...
### Comparison of Results for High Risk Loans
...
The summary chart above displays the results for high risk loan applications for easy comparison.  When considering whether to approve or not to approve a loan application based on risk, sensitivity is more important than precision.  It's more conservative to reject low risk loan applications than miss rejecting high risk loan applications.
  ...
  Although both of the Ensemble classifiers outperform the previous four machine learning models, the EasyEnsembleClassifier is the superior model for predicting high risk default in terms of accuracy, precision and sensitivity.
  * Confusion Matrix EasyEnsembleClassifier:
EASY ENSEMBLE | HIGH RISK PREDICTED TRUE | HIGH RISK PREDICTED FALSE | TOTAL |
-----------------|----------------|-----------------|-----------------|
HIGH RISK ACTUAL TRUE | TP = 93 | FN = 8 | HR (T) TOT = 375 |
HIGH RISK ACTUAL FALSE | FP = 983 | TN = 16121| HR (F) TOT = 68,470 |
  * Accuracy - EasyEnsembleClassifier has a 93% accuracy.  It outperforms the nearest model, Balanced Random Forest Classifier, by 14%.
